        ...
        try {
            ...
            socket.on("start_round", async () => {
                try {
                    ...
                    if (!roomId || !gameRooms.has(roomId)) {
                        console.log("[Recovery] Attempting to restore game state for start_round...");
                        const user = await user_model_1.User.findById(socket.user.id);
                        if (user) {
                            const registration = await registration_model_1.Registration.findOne({
                                members: { $all: [user.rollNumber] },
                            ...
                            if (registration) {
                                roomId = String(registration._id);
                                socket.join(roomId);
                                socketRoomMap.set(socket.id, roomId);
                                if (!gameRooms.has(roomId)) {
                                    console.log("[Recovery] Re-initializing game room from DB...");
                                    const teamUsers = await user_model_1.User.find({
                                        rollNumber: { $in: registration.members },
                                    });
                                    const players = teamUsers.map((u) => ({
                                        id: u._id.toString(),
                                        username: u.username,
                                        name: u.name,
                    ...
                    if (room) {
                        ...
                        const selectedMovie = (0, movies_1.getRandomMovie)(room.usedMovieIds);
            ...
            socket.on("check_active_game", async () => {
                ...
                try {
                    ...
                    if (activeGameRoom) {
                        console.log("[DEBUG] Player rejoining active game via check_active_game:", roomId);
                        const movieToGuess = activeGameRoom.currentMovie
                            .split(" ")
                            .map((word, idx) => {
                            if (activeGameRoom.revealedWords.includes(idx))
                                return word;
                            return "_".repeat(word.length);
                        })
                            .join(" ");
                        const revealedMap = {};
                        activeGameRoom.revealedLetters.forEach((idx) => {
                            const movieTitleNoSpaces = activeGameRoom.currentMovie.replace(/ /g, "");
                        ...
                        console.log("[DEBUG] Available rooms:", Array.from(gameRooms.keys()));
            ...
            socket.on("disconnect", async () => {
                ...
                if (gameRooms.has(roomId)) {
                    ...
                    if (room.isRoundActive) {
                        console.log(`[Disconnect] active game in room ${roomId}. Ending game.`);
                ...
                setTimeout(() => {
                    ...
                    if (!clientsInRoom || clientsInRoom.size === 0) {
                        if (gameRooms.has(roomId)) {
                            console.log(`[State Cleanup] Room ${roomId} is empty. Checking again...`);
